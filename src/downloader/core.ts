...
export class Downloader {
  ...
  public async start(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Worker thread error:', error as Error);
    } finally {
      this.isRunning = false;
  ...
  private async worker(workerId: number): Promise<void> {
    ...
    while (this.isRunning) {
      try {
        ...
        this.logger.error(`Worker ${workerId} encountered error:`, error as Error);
   ...
   */
  private async processDownloadItem(item: DownloadItem, workerId: number): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Worker ${workerId} error processing ${item.url}:`, error as Error);
      item.status = DownloadStatus.FAILED;
      item.endTime = new Date();
  ...
  private async downloadFile(item: DownloadItem): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error(`Download error for ${item.url}:`, error as Error);
      return false;
    }
  }
   ...
   */
  public async addDownload(url: string, filename?: string, size?: number): Promise<void> {
